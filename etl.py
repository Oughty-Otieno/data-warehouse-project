...
def load_staging_tables(cur, conn):
    """Load JSON input data (log_data and song_data) from S3 and insert
        into staging_logs and staging_songs tables.
    ...
    * conn --   parameters (host, dbname, user, password, port)
    ...
    Output:
    * log_data in staging_logs table.
    * song_data in staging_songs table.
    ...
    for query in copy_table_queries:
        ...
        conn.commit()
        print('------------------')
        print('{} processed successfuly.'.format(query))
...
def insert_tables(cur, conn):
    """Insert data from staging tables (staging_logs and staging_songs)
    ...
    * Data inserted from staging tables to fact table.
    ...
    for query in insert_table_queries:
        ...
        cur.execute(query)
        conn.commit()
        print('{} processed successfully.'.format(query))
